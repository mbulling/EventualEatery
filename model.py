...
def convert_features(date, day, hall):
    month = int(date[5:7])
    date_of_month = int(date[8:10])
    days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    weekday = days.index(day)
    pi = np.pi
    return [hall, np.sin(month*pi*2/12), np.cos(month*pi*2/12), np.sin(date_of_month*pi*2/31), np.cos(date_of_month*pi*2/31), np.sin(weekday*pi*2/7), np.cos(weekday*pi*2/7)]
...
def train_decision_tree(dinnerfood, tree):
    ...
    X = data[['Dining Hall','month_sin','month_cos','date_sin','date_cos', 'week_day_cos', 'week_day_sin']]
    Y= data.drop(columns=['Dining Hall','month_sin','month_cos','date_sin','date_cos', 'week_day_cos', 'week_day_sin','month','date','week_day','Date','Day'])
...
print(predict_menu(getFood(), convert_features('2018-05-19','Saturday', 4)))
